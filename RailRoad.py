...
class Act():
    ...
    def GetlineInfo(self, index):
        return self.lineInfo[index]
    
    def GetlineInfoX(self, index):
        st = self.lineInfo[index]
        return st[0]
    
    def GetlineInfoY(self, index):
        st = self.lineInfo[index]
        return st[1]
    
    def GetlineInfoZ(self, index):
        st = self.lineInfo[index]
        return st[2]
    
    
    def GetlinePointXst(self, index):
        StraightList = self.lineInfo[index]
    ...
    def GetCurvePoint(self, t):
        if t == 1:
            return self.pathInfo[len(self.pathInfo)-1]
        else:
            cp = self.pathInfo
    ...
    def CurveA(self, t):
        if len(self.pathInfo) == 0:
            pass
        elif len(self.pathInfo) == 1:
            pass
        elif len(self.pathInfo) == 2:
            return Vector3.Lerp(self.pathInfo[0], self.pathInfo[1], t)
        elif len(self.pathInfo) == 3:
            return self.GetPoint(t, self.pathInfo[0], self.pathInfo[1], self.pathInfo[2])
        else: # N차 베지어 곡선
    ...
    def GetCurvePointHelf(self, t):
        if t == 1:
            ...
            cp = self.pathInfo
            cunnetLines = len(self.pathInfo)-2

            if (len(self.pathInfo) % 2) == 0:
                while cunnetLines > 2:
                    cp2 = []
                    for c in range(cunnetLines):
    ...
    def Closed(self):
        self.pathInfo.append(self.pathInfo[0])
